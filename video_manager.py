...
class VideoManager(wx.Frame):
    ...
    def __init__(self, parent, id, title, mp4box, categories=["1", "2", "3", "4", "5","split"], input_directory=None, review_folder=None):
        ...
        self.editMenuItemUndo = self.editMenu.Append(wx.ID_ANY, text="Undo")
        self.editMenuItemRedo = self.editMenu.Append(wx.ID_ANY, text="Redo")
        ...
        self.helpMenuAboutItem = self.helpMenu.Append(-1, text='&About')
        ...
        playButtonImg = self.getBmpFromSvg(resource_string(__name__, os.path.join("resources", "icons", 'play-button.svg')), icon_size_default, icon_size_default)
        ...
        stopButtonImg = self.getBmpFromSvg(resource_string(__name__, os.path.join("resources", "icons", 'stop-button.svg')), icon_size_default, icon_size_default)
...
def __generate_window_title__(title):
    if title is None:
        ret_value = "%s %s" % (video_splitter_globals.app_name, video_splitter_globals.app_version)
    else:
        ret_value = "%s - %s %s" % (title, video_splitter_globals.app_name, video_splitter_globals.app_version)
    return ret_value
